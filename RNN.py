...
if train_for_specific_config:
    ...
    for i in range(len(y_pred)):
       ...
       if normalize_flag:
           ...
           prediction_values.append(y_pred[i].flatten()[0])
           actual_values.append(y_test[i][timesteps-1].reshape(-1, 1)[0][0])
           
           print(y_pred[i].flatten()[0], ",", y_test[i][timesteps-1].reshape(-1, 1)[0][0])
